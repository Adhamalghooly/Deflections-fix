...
const Index = () => {
  ...
  const [viewFilter, setViewFilter] = React.useState<'moments' | 'deflection' | 'none'>('none');
  ...
  const getDesignBeamName = useCallback((d: any) => {
    if (d.mergedCarrierIds && d.mergedCarrierIds.length > 0) {
      const parts = d.mergedCarrierIds.map((id: string) => beamsWithLoads.find(b => b.id === id)).filter(Boolean);
      if (parts.length > 0) {
        const customPart = parts.find((p: any) => p.name);
        if (customPart && customPart.name) {
          const match = customPart.name.match(/^(.+)-(\d+)$/);
    ...
    if (beam) return beam.name ?? beam.id;
    return d.beamId;
  }, [beamsWithLoads]);
  ...
  const beamDesigns = useMemo(() => {
    ...
    if (designSource === 'etabs' && (designExecuted || analyzed) && etabsAnalysisData.length > 0) {
      const designs: {
  ...
  const handleSelectBeamForViewingAndEditing = useCallback((targetBeam: typeof beams[0]) => {
    if (targetBeam.storyId) {
      setViewStoryId(targetBeam.storyId);
      dispatch({ type: 'SELECT_STORY', storyId: targetBeam.storyId });
    }
    
    // Call pre-existing, robust, coordinate-aware matching long-press function
    handleLevelElementLongPress('beam', targetBeam.id);
  }, [setViewStoryId, dispatch, handleLevelElementLongPress]);
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden" style={{ marginBottom: 'calc(var(--nav-height) + var(--safe-bottom))' }}>
        <Tabs value={activeTab} onValueChange={tab => dispatch({ type: 'SET_ACTIVE_TAB', tab })} className="h-full flex flex-col">
          ...
          {/* VIEW TAB */}
          <TabsContent value="view" className="flex-1 overflow-y-auto p-3 md:p-4 mt-0 pb-20 md:pb-4 overflow-visible">
                    ...
                    {analyzed && (
                      <div className="flex items-center gap-2 flex-wrap">
                        <div className="flex items-center gap-1">
                          <span className="text-[11px] font-medium text-muted-foreground">محتوى المخطط:</span>
                          <select
                            className="h-7 rounded border border-input bg-background px-2 text-[11px] min-w-[145px]"
                            value={viewFilter}
                            onChange={e => {
                              const val = e.target.value as 'moments' | 'deflection' | 'none';
                              setViewFilter(val);
                              setShowViewMoments(val === 'moments');
                            }}
                          >
                            <option value="none">الافتراضي (حالة الإجهاد)</option>
                            <option value="moments">مخطط العزوم (Moments)</option>
                            <option value="deflection">تحليل الترخيم (Deflection)</option>
                          </select>
                        </div>
                        {viewFilter === 'moments' && (
                          <select
                      ...
                      <TableBody>
                        {stories.map(story => 
                          ...
                          <TableRow className="cursor-pointer" onClick={() => handleSelectElement('beam', d.beamId)}>
                            <TableCell className="text-xs font-medium text-muted-foreground">{story.label}</TableCell>
                            <TableCell className="font-mono text-xs">{getDesignBeamName(d)}</TableCell>
                          ...
                          {diag && !diag.isAdequate && diag.failures.map((f, idx) => {
                            ...
                            if (f.type === 'deflection') {
                              ...
                              if (beamForDefl) {
                                ...
                                const actualEndCondition = (d as any).endCondition || 'both-ends';
                                ...
                                for (let hTry = beamForDefl.h + 25; hTry <= beamForDefl.h * 4 && hTry <= 2500; hTry += 25) {
                                  ...
                                  const testDefl = calculateDeflection(span, bw, hTry, mat.fc, wD, wL, testAs, actualEndCondition, 'B', testAs * 0.3, 1.0, 60);
                                ...
                                if (hReq > beamForDefl.h) {
                                  ...
                                  const testWithMoreSteel = calculateDeflection(span, bw, beamForDefl.h, mat.fc, wD, wL, moreAs, actualEndCondition, 'B', moreAs * 0.35, 1.0, 60);
                          ...
                          beamDesigns.filter(d => {
                            ...
                            return (
                              <TableRow key={`as-${story.id}-${d.beamId}`} className="cursor-pointer" onClick={() => handleSelectElement('beam', d.beamId)}>
                                ...
                                <TableCell className="font-mono text-xs font-bold">{getDesignBeamName(d)}</TableCell>
      ...
      {momentChartElement && (
        <ElementMomentChartModal
          ...
          colDesigns={colDesigns}
          onSaveBeamProps={(beamId, updates) => {
            const isExtra = extraBeams.some(eb => eb.id === beamId);
            if (isExtra) {
              dispatch({
                type: 'UPDATE_EXTRA_BEAM',
                id: beamId,
                updates: {
                  name: updates.name,
                  b: updates.b,
