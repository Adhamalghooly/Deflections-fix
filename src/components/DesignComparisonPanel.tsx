...
export default function DesignComparisonPanel({
  ...
  const getBeamNameForComparison = (beamId: string) => {
    const partIds = splitBeamGroups[beamId] || [beamId];
    const repBeam = beams.find(b => b.id === beamId) || beams.find(b => partIds.includes(b.id));
    if (repBeam) {
      if (repBeam.name) {
        if (splitBeamGroups[beamId]) {
          const match = repBeam.name.match(/^(.+)-(\d+)$/);
                ...
                <TableBody>
                  {beamComparisons.map(row => (
                    <TableRow key={`${row.storyLabel}-${row.beamId}`}>
                      ...
                      <TableCell className="font-mono text-[10px] font-bold">{getBeamNameForComparison(row.beamId)}</TableCell>
