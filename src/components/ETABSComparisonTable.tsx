...
const ETABSComparisonTable: React.FC<Props> = ({
  ...
  const getBeamDisplayName = useCallback((beamId: string) => {
    ...
    const parts = splitBeamPartsMap.get(beamId);
    if (parts && parts.length > 0) {
      const firstPart = parts[0].beam;
      if (firstPart && firstPart.name) {
        const match = firstPart.name.match(/^(.+)-(\d+)$/);
        if (match) return match[1];
        return firstPart.name;
      }
    }
    return beamId;
  }, [beams, splitBeamPartsMap]);
                ...
                <Button
                  ...
                  onClick={() => {
                    const rows = beamRows.map(r => {
                      const etabs = etabsMap.get(r.beamId);
                      return [
                        r.storyLabel, r.frameId, getBeamDisplayName(r.beamId), r.span.toFixed(2),
